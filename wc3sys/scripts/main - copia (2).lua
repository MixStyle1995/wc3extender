...
function main()
    ...
    local function spawn_hero(unitId, name)
		...
		--CeSetCursorMode(9, "ReplaceableTextures\\Selection\\SpellAreaOfEffect.blp", 1)
		--CeSetCursorSelectionScale(0.5) 
		
        local unit = CreateUnit(player, fourcc(unitId), 0.0, 0.0, 270.0)
        if unit and unit ~= 0 then
            SetHeroLevel(unit, 10, false)
			--CeApplyBuffToUnit(unit, fourcc('Bslo'), 5.0, unit) 
			--CeVisualRootsUnit(unit, 5.00, unit)
			CeVisualBuffUnit(unit, fourcc('BEer'), 3.00, unit)
			CeVisualBuffUnit(unit, fourcc('Bslo'), 6.00, unit)
            --local stunned = CeSlowUnit(unit, 5.0, -0.60, -0.25, unit)
			--CeStunUnit(unit, 5.0, unit)
            print("Cheat console spawned " .. name .. " at level 10; slow=" .. tostring(stunned))
        else
