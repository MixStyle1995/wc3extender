...
pub fn make_jass_string(c_string: *const u8) -> i32 {
    ...
    unsafe {
        let jass_vm = *(addrs.jass.jass_vm_global as *const *const c_void);

        if jass_vm.is_null() {
            return 0;
        }

        let f: MakeJassStringFn = core::mem::transmute(addrs.jass.make_jass_string);
        f(jass_vm, c_string)
    }
}
...
pub fn jass_string_to_str(handle: i32) -> Option<String> {
    ...
    unsafe {
        let f: JassStringToCStrFn = core::mem::transmute(addrs.jass.jass_string_to_cstr);
...
pub unsafe fn jass_instance_from_index(instance_index: u32) -> Option<usize> {
    ...
    unsafe {
        let f: JassInstanceFromIndexFn = core::mem::transmute(addrs.jass.jass_instance_from_index);
...
pub fn jass_string_handle_to_arg(handle: u32, instance_index: u32) -> Option<usize> {
    ...
    unsafe {
        ...
        let f: JassStringHandleToArgFn = core::mem::transmute(addrs.jass.jass_string_handle_to_arg);
...
pub fn string_to_arg(c_string: *const u8, instance_index: u32) -> Option<usize> {
    ...
    unsafe {
        let from_cstr: JassStringHandleFromCStrByIndexFn =
            core::mem::transmute(addrs.jass.jass_string_handle_from_cstr_by_index);
        let to_arg: JassStringArgFromHandleByIndexFn =
            core::mem::transmute(addrs.jass.jass_string_arg_from_handle_by_index);
...
pub fn register_native(name: &CStr, signature: &CStr, function: *const c_void) -> i32 {
    ...
    unsafe {
        let f: RegisterNativeFn = core::mem::transmute(addrs.jass.register_native);
